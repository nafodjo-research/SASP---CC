...
Table~\ref{tab:mde} reports minimum detectable effects at 80\% power and a 5\% two-sided test, computed as $(z_{0.975} + z_{0.80})$ times the realised clustered standard error. These convert null results into statements about what the data can and cannot rule out, and the comparison across samples is stark. On the pooled ATP sample the minimum detectable ranking-point effect is 74.8 at 4 weeks, 74.4 at 8 weeks, and 73.1 at 12 weeks, each sitting just below the estimated effect of 72.5, 72.1, and 82.8; the design has approximately the power required to detect effects of the size it reports, and no more. On the verified-lottery ATP subsample the minimum detectable effect rises to 92 to 125 over the same range while the point estimates fall, so a null there is close to uninformative. The pattern is sharper still on the WTA side, where the verified subsample requires 95 to 129 points at short horizons.
