...
Table~\ref{tab:lee} reports \citet{Lee2009_bounds} bounds, which trim the over-observed group from each tail of its outcome distribution to equalize observation rates and so bound the effect for the subpopulation observed under either assignment. Two features of the table govern how much it can be asked to carry. The quantity bounded is the raw difference in mean ranking-point change between treated and control players, not the regression-adjusted coefficients reported in Tables~\ref{tab:dynamic_atp} and~\ref{tab:dynamic_wta}; the two differ because the headline specification absorbs Grand Slam $\times$ Year effects and pre-treatment covariates that the raw difference does not. And where observation rates are already equal, no trimming occurs and the bounds collapse to a point, which is what produces the degenerate WTA entries at 12 and 26 weeks.

On the ATP tour the bounds are tight and positive through 26 weeks: the 12-week raw difference of 44.4 ranking points is bounded within $[38.3, 50.1]$, and the 26-week difference of 37.2 within $[23.4, 46.8]$. Differential survival cannot generate the short- and medium-horizon ATP result. The WTA tour supports no such statement. Its 4- and 8-week bounds are positive ($[23.8, 23.8]$ and $[32.4, 44.1]$), but the 12- and 26-week entries are degenerate points of $10.8$ and $-11.5$, the latter negative, so the WTA evidence at medium horizons is that there is nothing to bound rather than that a positive effect survives bounding. At 52 weeks the bounds widen sharply and span zero on both tours ($[-143.9, 35.3]$ ATP; $[-18.2, 90.7]$ WTA), because the trimming fraction rises to 17\% and 12\% respectively. The paper's fade-out claim at 52 weeks is consequently the one result that differential attrition can also explain, and Section~\ref{sec:conclusion} reports it with that caveat rather than as established.
...
Romano-Wolf step-down inference is stricter and gives a different answer. Where Benjamini-Hochberg bounds the expected share of rejections that are false, Romano-Wolf bounds the probability of making any false rejection at all, and it does so using a null-imposed wild cluster bootstrap that preserves the dependence among the test statistics \citep{RomanoWolf2005_stepdown, Clarke2020_romanowolf}. Of the 80 tests, 16 survive at the 5\% level, and all 16 are non-Grand-Slam access outcomes: main draws entered and matches at 250-level events, at 4, 8, 26 and 52 weeks on both tours, where the raw $p$-values fall below $0.001$. No Grand Slam ranking-point effect survives. On the ATP tour the step-down $p$-values are $0.348$ at 4 weeks, $0.348$ at 8 weeks, and $0.130$ at 12 weeks; the WTA 8-week effect returns $0.558$.
...
Table~\ref{tab:wildboot} reports wild cluster bootstrap $p$-values following \citet{Cameron2008_wildbootstrap}, using the restricted bootstrap with the null imposed and Webb six-point weights, which outperform Rademacher weights when the cluster count is small. The bootstrap re-estimates the full specification 9{,}999 times per coefficient, and is computed with the \texttt{fwildclusterboot} implementation of \citet{Roodman2019_fastwild}.

The bootstrap corroborates the asymptotic inference rather than overturning it, and it tracks the asymptotic $p$-values closely at every horizon on every sample. On the pooled ATP sample the bootstrap values at 4, 8, and 12 weeks are 0.006, 0.009, and 0.002 against asymptotic values of 0.008, 0.008, and 0.002. On the pooled WTA sample the 8-week bootstrap value is 0.010 against 0.017, and on the verified WTA subsample it is 0.040 against 0.044. One coefficient crosses the conventional threshold: the 26-week ATP effect moves from $p = 0.047$ to $p_{\text{boot}} = 0.052$. Everywhere else the two procedures agree on every rejection and every failure to reject. Cluster-robust standard errors are therefore not materially over-rejecting here, which is consistent with the cluster counts in Table~\ref{tab:clusters} sitting above the range where the small-cluster problem is acute.

The 26-week ATP effect is accordingly the weakest of the estimates the paper calls significant, and the three inference procedures line up against it two to one. It clears the 5\% threshold under clustered standard errors ($p = 0.047$) and fails under both the wild bootstrap ($p_{\text{boot}} = 0.052$) and Fisher randomization inference ($p_{\text{Fisher}} = 0.103$). The gap between the bootstrap and the randomization test is itself informative about what each procedure assumes. Both bootstrap and asymptotic inference treat the regression as correctly specified and resample from it, whereas the Fisher test permutes the treatment indicator within the blocks in which it was actually assigned and so tests the sharp null using the design itself. For a lottery, randomization inference is the more appropriate standard, and the 26-week result fails it by the widest margin of the three procedures. The paper reports the estimate but does not rest any claim on it.
...
\begin{tablenotes}
\footnotesize
\item \textit{Notes.} Restricted wild cluster bootstrap with the null imposed, Webb six-point weights, and 9{,}999 replications, clustered at the player level \citep{Cameron2008_wildbootstrap}, computed with the \texttt{fwildclusterboot} implementation of \citet{Roodman2019_fastwild}. Clustered $p$-values are the asymptotic values reported elsewhere in the paper.
\end{tablenotes}
\end{threeparttable}
\end{table}
